...
CREATE TABLE [staging].[ProductSubCategory] (
    [StagingProductSubCategoryKey] INT IDENTITY(1,1) NOT NULL,
    [SourceProductSubCategoryID] INT NOT NULL,
    [Name] NVARCHAR(50) NOT NULL,

    CONSTRAINT [pk_staging_ProductSubCategory_StagingProductSubCategoryKey] PRIMARY KEY CLUSTERED ([StagingProductSubCategoryKey] ASC),
    CONSTRAINT [uk_staging_ProductSubCategory_SourceProductSubCategoryID] UNIQUE ([SourceProductSubCategoryID])
);
GO
...
-- Source bridge tables used only for staging-to-work relationship resolution.
CREATE TABLE [staging].[BusinessEntityAddress] (
    [StagingBusinessEntityAddressKey] BIGINT IDENTITY(1,1) NOT NULL,
    [SourceBusinessEntityID] INT NOT NULL,
    [SourceAddressID] INT NOT NULL,
    [SourceAddressTypeID] INT NOT NULL,

    CONSTRAINT [pk_staging_BusinessEntityAddress_StagingBusinessEntityAddressKey] PRIMARY KEY CLUSTERED ([StagingBusinessEntityAddressKey] ASC),
    CONSTRAINT [uk_staging_BusinessEntityAddress_SourceRelationship] UNIQUE ([SourceBusinessEntityID], [SourceAddressID], [SourceAddressTypeID])
);
GO

CREATE TABLE [staging].[SpecialOfferProduct] (
    [StagingSpecialOfferProductKey] BIGINT IDENTITY(1,1) NOT NULL,
    [SourceSpecialOfferID] INT NOT NULL,
    [SourceProductID] INT NOT NULL,

    CONSTRAINT [pk_staging_SpecialOfferProduct_StagingSpecialOfferProductKey] PRIMARY KEY CLUSTERED ([StagingSpecialOfferProductKey] ASC),
    CONSTRAINT [uk_staging_SpecialOfferProduct_SourceRelationship] UNIQUE ([SourceSpecialOfferID], [SourceProductID])
);
GO

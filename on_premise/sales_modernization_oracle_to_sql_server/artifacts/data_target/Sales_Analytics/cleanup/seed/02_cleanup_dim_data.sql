/* WARNING: LOCAL/DEV DATA CLEANUP ONLY. Deletes all dimension rows. Order: 02. */
USE [Sales_Analytics];
GO
DECLARE @sql NVARCHAR(MAX)=N'';
SELECT @sql+=N'DELETE '+QUOTENAME(s.name)+N'.'+QUOTENAME(t.name)+N';'+CHAR(13)
FROM sys.tables t JOIN sys.schemas s ON s.schema_id=t.schema_id WHERE s.name=N'dim';

/*============================================================================
  ...
  Scenario: Sales_Operational_Migration - Successful Full Flow

  Purpose:
  - Simulates a successful execution of the full Sales_Operational_Migration flow.
  ...
  Cleanup:
  - Run 00_cleanup_sales_operational_success_full_flow.sql to remove test data.
============================================================================*/
...
DECLARE @scenario_name VARCHAR(100) = 'SALES_OPERATIONAL_SUCCESS_FULL_FLOW';
...
VALUES
    (1, 'Reference Data Load'),
    (2, 'Master Data Load'),
    (3, 'Transactional Data Load');
...
VALUES
    ('Reference Data Load',     1, 'AddressType Load',     10,      20),
    ('Reference Data Load',     2, 'ProductCategory Load', 50,      30),
    ('Reference Data Load',     3, 'SpecialOffer Load',    100,     45),
    ('Reference Data Load',     4, 'ShipMethod Load',      20,      30),
    ('Reference Data Load',     5, 'Geography Load',       1000,    180),
    ('Reference Data Load',     6, 'Currency Load',        200,     45),
    ('Reference Data Load',     7, 'CreditCard Load',      100000,  240),

    ('Master Data Load',        1, 'Address Load',         185000,  300),
    ('Master Data Load',        2, 'Product Load',         50000,   240),
    ('Master Data Load',        3, 'SalesPerson Load',     5000,    120),
    ('Master Data Load',        4, 'Customer Load',        150000,  360),

    ('Transactional Data Load', 1, 'Sales Load',           1500000, 1200);
...
AND [name] = 'Sales_Operational_Migration';

IF @root_project_process_id IS NULL
    THROW 51002, 'Sales_Operational_Migration process was not found.', 1;
...
BEGIN
    ...
    IF @group_project_process_id IS NULL
        THROW 51003, 'A process group in the Sales_Operational_Migration flow was not found.', 1;
    ...
    BEGIN
        ...
        IF @project_process_id IS NULL
            THROW 51004, 'A leaf process in the Sales_Operational_Migration flow was not found.', 1;

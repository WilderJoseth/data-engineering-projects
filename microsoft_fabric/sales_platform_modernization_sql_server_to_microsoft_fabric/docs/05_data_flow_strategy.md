...
The historical reporting flow loads trusted historical reporting data from `Sales_Analytics` into Fabric.
...
| Purpose             | Initialize Fabric with historical reporting data |
...
| Support reconciliation                 | Allow comparison between `Sales_Analytics` and Fabric targets                            |
| Prepare for reporting cutover          | Ensure Fabric contains the required historical baseline before reporting ownership moves |

## New Reporting Data Flow

The new reporting data flow loads new transactional data from `Sales_Operational` and transforms it into reporting-ready structures in Fabric.
...
| Support future reporting    | Gold becomes the trusted source for new Fabric reporting periods                          |

## Coexistence Support Flow

Coexistence support is required while the organization transitions from on-premise reporting to Fabric reporting.

During this phase, `Sales_Analytics` may still be available as the existing reporting baseline, while Fabric is being loaded, reconciled, tested, and prepared for reporting cutover.

| Coexistence Need      | Description                                                                                 |
| --------------------- | ------------------------------------------------------------------------------------------- |
| Historical comparison | Compare Fabric historical results against `Sales_Analytics`                                 |
| Reporting validation  | Confirm Fabric Gold and semantic model outputs match the accepted business baseline         |
| Temporary fallback    | Keep `Sales_Analytics` available until Fabric reporting is accepted                         |
| Period control        | Avoid loading the same reporting period from both `Sales_Analytics` and `Sales_Operational` |

Coexistence should be temporary. After cutover, Fabric should become the reporting source of truth.

## Flow Ownership Rules

| Rule                                                     | Description                                                                 |
| -------------------------------------------------------- | --------------------------------------------------------------------------- |
| `Sales_Operational` remains operational source of truth  | Fabric does not replace on-premise transactional processing                 |
| `Sales_Analytics` provides historical reporting baseline | Historical data is loaded from the trusted analytical source                |
| `Sales_Operational` provides new operational data        | New reporting periods are derived from operational data through Fabric      |
| Gold owns reporting output                               | Reports should consume Gold through the semantic model                      |
...
| `Sales_Analytics` is trusted for historical reporting | Historical data is treated as the baseline for Fabric initialization                                      |
...
`Sales_Analytics` seeds Fabric with trusted historical reporting data through Warehouse staging and Gold. `Sales_Operational` provides new operational data that is landed in Bronze, curated in Silver, and transformed into Gold. `wh_sales_analytics.gold` becomes the trusted analytical layer for reporting through `sm_sales_analytics`.

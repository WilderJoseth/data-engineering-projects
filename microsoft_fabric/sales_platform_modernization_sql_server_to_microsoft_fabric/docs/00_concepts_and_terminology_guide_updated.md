...
This guide keeps terminology consistent across the **Sales Platform Modernization: SQL Server to Microsoft Fabric** project.

## Architecture Scope

The current platform runs on an on-premise SQL Server 2022 environment with two main databases.

| Database | Role |
|---|---|
| `Sales_Operational` | Operational source database and operational system of record for active sales transactional processing |
| `Sales_Analytics` | Current reporting source of truth and historical reporting baseline |

The target analytics platform uses Microsoft Fabric and Azure SQL Database.

| Component | Proposed Name | Role |
|---|---|---|
| Fabric Lakehouse | `lh_sales_operational` | Stores Bronze and Silver operational data areas used to prepare new reporting data from `Sales_Operational` |
| Fabric Warehouse | `wh_sales_analytics` | Stores staging and Gold analytical data areas used to build the governed reporting model |
| Power BI Semantic Model | `sm_sales_analytics` | Provides governed reporting consumption, business measures, relationships, and report-facing access |
| Azure SQL Database | `DataOps_Control` | Provides metadata-driven execution control, observability, validation, reconciliation, error logging, and rerun support |

---

## Modernization Scope

This project modernizes the reporting and analytics platform.

It does **not** replace the operational sales system.

`Sales_Operational` remains the **operational system of record** for active sales transactions. New reporting data is extracted from this source, processed through the Fabric Lakehouse, and then curated into the Fabric Warehouse Gold model.

`Sales_Analytics` represents the **current reporting source of truth** before modernization. It provides trusted historical reporting data used as the **historical reporting baseline** for initializing, validating, and reconciling the target analytical model.

After cutover, the Fabric Warehouse Gold model, exposed through the Power BI semantic model, becomes the **target reporting source of truth** for governed reporting and analytics.
...
| Low-level | Describes concrete implementation objects | Schema, table, view, stored procedure, pipeline, notebook, semantic model table | `bronze.SalesOrderHeader`, `silver.SalesOrders`, `gold.Factsales`, `gold.DimCustomer` |
...
| Source object | Low | A table, view, or query read from a source database | Discussing ingestion mappings, dependencies, or reconciliation | Referring to the whole database or platform | `Sales_Operational.dbo.SalesOrderHeader` | Source objects are mapped to target Lakehouse or Warehouse objects. |
| Data source | Generic / implementation | Any specific input used by a pipeline, notebook, or process | Referring to a specific table, view, query, file, or API used as input | Referring to the whole SQL Server platform when a more precise term exists | A query over `Sales_Analytics.dbo.FactSales` | Use data source only for specific pipeline inputs. |
...
| Target analytics platform | High | The complete modern reporting and analytics architecture | Referring to the whole destination architecture | Referring to a specific Lakehouse table, Warehouse table, or semantic model object | Fabric Lakehouse, Fabric Warehouse, Semantic Model, and DataOps Control database | The target analytics platform modernizes reporting and analytics using Microsoft Fabric and Azure SQL Database for DataOps control. |
...
| Silver | Lakehouse | Curated operational data area | Standardizes, validates, and prepares Bronze data | `silver.SalesOrders` |
...
> Historical reporting data is loaded from the current reporting source of truth, `Sales_Analytics`, into the Fabric Warehouse staging area. It is then curated into the Warehouse Gold model and exposed through the Power BI semantic model.
...
| Direct Gold access | Reports query Gold tables directly | Technically possible but not preferred for governed reporting | Report connects directly to `gold.fact_sales` |
...
## Standard Project Wording

Use the following wording consistently across project documentation:

> The source platform is the on-premise SQL Server 2022 environment. It contains two main source databases: `Sales_Operational`, which remains the operational system of record for active sales transactional processing, and `Sales_Analytics`, which is the current reporting source of truth and provides the historical reporting baseline.
>
> The target analytics platform is based on Microsoft Fabric and supported by Azure SQL Database for DataOps control. The Fabric Lakehouse `lh_sales_operational` stores Bronze and Silver operational data areas for new reporting data. The Fabric Warehouse `wh_sales_analytics` stores staging and Gold analytical data areas, including the curated reporting model. The Power BI semantic model `sm_sales_analytics` provides governed reporting consumption. The Azure SQL Database `DataOps_Control` supports metadata-driven execution control, observability, validation, reconciliation, error logging, and rerun support.
>
> After cutover, the Fabric Warehouse Gold model, exposed through the Power BI semantic model, becomes the target reporting source of truth for governed reporting and analytics.
>
> This project modernizes reporting and analytics. It does not replace the operational sales system.
...
| Use “target analytics platform” for the overall target architecture | This is more precise than saying Fabric every time |
| Use “source platform” for the full SQL Server 2022 environment | This avoids reducing the current platform to a generic data source |
| Use “data source” only for specific pipeline inputs | A data source can be a table, view, query, file, or API |
| Use “schema” for `bronze`, `silver`, `staging`, and `gold` when discussing implementation | These are implementation groupings inside Lakehouse or Warehouse |
